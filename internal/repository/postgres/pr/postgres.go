...
func (s *DB) CreatePR(prID, name, authorID string, reviewers []string) error {
	_, err := s.sql.Exec(`
...
func (s *DB) GetPR(prID string) (*modelpr.PullRequest, error) {
...
func (s *DB) UpdateReviewers(prID string, reviewers []string) error {
	_, err := s.sql.Exec(`UPDATE pull_requests SET assigned_reviewers = $1 WHERE id = $2`, reviewers, prID)
	return err
}

func (s *DB) MergePR(prID string) (*modelpr.PullRequest, error) {
	now := time.Now()
...
func (s *DB) GetPRsByReviewer(userID string) ([]modelpr.PullRequestShort, error) {
...
func (s *DB) PRExists(prID string) (bool, error) {
	var exists bool
